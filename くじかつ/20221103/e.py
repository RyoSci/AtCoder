...
ans = 0
for l in range(n):
    # while r < n and len(tree) > 0 and not (tree[0] == y and tree[-1] == x):
    while r < n:
        if len(tree) > 0 and tree[0] == y and tree[-1] == x:
            break
        if len(tree) == 0 or len(tree) > 0 and tree[0] >= y and tree[-1] <= x:
            tree.add(a[r])
            r += 1
        if len(tree) > 0 and (tree[0] < y or tree[-1] > x):
            break

    if len(tree) > 0 and (tree[0] == y and tree[-1] == x):
        pos = bisect_left(stop, r)
        ans += stop[pos]-r+1

    tree.discard(a[l])
    if len(tree) == 0 and r < n:
        tree.add(a[r])
        r += 1
