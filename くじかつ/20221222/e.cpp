...
int main() {
    ...
    vector dp(n, vector(m, vector(2, INF)));
    dp[0][0][0] = 1;
    if (a[0] == b[0]) dp[0][0][1] = 0;

    rep(i, n) {
        rep(j, m) {
            if (i > 0 and j > 0) {
                if (a[i] == b[j]) {
                    chmin(dp[i][j][1], dp[i - 1][j - 1][0]);
                    chmin(dp[i][j][1], dp[i - 1][j - 1][1]);
                }
                chmin(dp[i][j][0], dp[i - 1][j - 1][0] + 1);
                chmin(dp[i][j][0], dp[i - 1][j - 1][1] + 1);
            }
            if (i > 0) {
                if (a[i] == b[j]) {
                ...
                chmin(dp[i][j][0], dp[i - 1][j][0] + 1);
                chmin(dp[i][j][0], dp[i - 1][j][1] + 1);
            }
            if (j > 0) {
                if (a[i] == b[j]) chmin(dp[i][j][1], dp[i][j - 1][0]);
