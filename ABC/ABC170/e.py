...
pos = [0]*n
rate = [0]*n
ele = [SortedList() for _ in range(2*10**5+10)]
for i in range(n):
...
for i in range(q):
    ...
    now_pos = pos[c]
    now_rate = rate[c]
    if ele[now_pos][-1] == now_rate:
        ave.discard(now_rate)
        ele[now_pos].discard(now_rate)
        if len(ele[now_pos]) > 0:
            ave.add(ele[now_pos][-1])
    else:
        ele[now_pos].discard(now_rate)

    if len(ele[d]) == 0:
        ave.add(now_rate)
    elif ele[d][-1] < now_rate:
        ave.discard(ele[d][-1])
        ave.add(now_rate)
    ele[d].add(now_rate)
