...
# https://atcoder.jp/contests/abc151/submissions/23117032
...
def f(r, cx, cy):
    for i in range(n):
        ...
        d = vector(*get_vector((x[i], y[i]), (cx, cy))).norm()
        if d > r:
            # if d - eps > r:
            return False
    return True
...
for i in range(n):
    ...
    for j in range(i):
        ...
        for k in range(j):
            ...
            c = get_circumcenter((xi, yi), (xj, yj), (xk, yk))
            if c is None:
                continue
            cx, cy = c
            r = vector(*get_vector(c, (xi, yi))).norm()
...
for i in range(n):
    for j in range(i):
        cx, cy = mid_point((x[i], y[i]), (x[j], y[j]))
        r = vector(*get_vector((cx, cy), (x[i], y[i]))).norm()
