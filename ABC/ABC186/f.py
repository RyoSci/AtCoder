...
tate = [[h] for _ in range(w)]  # 縦方向に障害物を入れる
yoko = [[w] for _ in range(h)]  # 横方向に障害物を入れる
...
canh = bisect_left(tate[0], 0)
canw = bisect_left(yoko[0], 0)
...
for i in range(canw):
    ...
    pos = bisect_left(tate[i], 0)
    pos = tate[i][pos] - 1
    cnt = arrows.bisect_right(pos)
